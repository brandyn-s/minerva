...
export function probe(store:ExperimentStore,raw:z.infer<typeof probeSchema>){
 const input=probeSchema.parse(raw),candidate=store.scopedRecord<Candidate>(input.runId,input.candidateId);if(!candidate)throw new Error("Candidate unavailable");
 let remaining=input.capacity;
 const allocations=input.requests.map(request=>{const allocated=Math.min(request,input.model==="central-queue"?remaining:input.capacity);if(input.model==="central-queue")remaining-=allocated;return allocated;});
 ...
 store.put("probe",result.id,input.runId,result);return result;
}

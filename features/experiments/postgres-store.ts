...
export class PostgresStore {
  ...
  async installSelectionGuard(){await this.query(`
    ...
    BEGIN
      IF (OLD.value->'selection' IS NOT NULL OR NEW.value->'selection' IS NOT NULL)
        AND current_setting('minerva.selection_writer',true) IS DISTINCT FROM 'reviewed-lens-v1' THEN
  ...
  async transaction<T>(fn:()=>T):Promise<Awaited<T>> {
    ...
    try{return await this.context.run(client,async()=>{await client.query('BEGIN');try{await client.query("SELECT set_config('minerva.selection_writer','reviewed-lens-v1',true)");const value=await fn();await client.query('COMMIT');return value;}catch(error){await client.query('ROLLBACK');throw error;}});}finally{client.release();}
  }

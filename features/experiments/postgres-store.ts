...
export class PostgresStore {
  ...
  async initialize(){await this.query(`CREATE SCHEMA IF NOT EXISTS expedition;
    ...
    CREATE TABLE IF NOT EXISTS expedition.interventions(seq bigserial PRIMARY KEY,id text UNIQUE NOT NULL,run_id text NOT NULL,value jsonb NOT NULL);`);await this.installSelectionGuard();}
  // Durable Workflows remain pinned to old deployments. Those writers must fail
  // before reserving another call or overwriting an applied population.
  async installSelectionGuard(){await this.query(`
    CREATE OR REPLACE FUNCTION expedition.require_selection_writer() RETURNS trigger LANGUAGE plpgsql AS $$
    BEGIN
      IF (OLD.value->'selection' IS NOT NULL OR NEW.value->'selection' IS NOT NULL)
        AND COALESCE(current_setting('minerva.selection_writer',true),'') NOT IN ('reviewed-lens-v1','group-weave-v1') THEN
        RAISE EXCEPTION 'This expedition requires the current selection worker' USING ERRCODE='55000';
      END IF;
      IF (OLD.value->'groupWeave' IS NOT NULL OR NEW.value->'groupWeave' IS NOT NULL)
        AND current_setting('minerva.selection_writer',true) IS DISTINCT FROM 'group-weave-v1' THEN
        RAISE EXCEPTION 'This expedition requires the current group Weave worker' USING ERRCODE='55000';
      END IF;
      RETURN NEW;
    END $$;
    CREATE OR REPLACE TRIGGER selection_writer BEFORE UPDATE ON expedition.runs
      FOR EACH ROW EXECUTE FUNCTION expedition.require_selection_writer();`);}
  ...
  async transaction<T>(fn:()=>T):Promise<Awaited<T>> {
    ...
    try{return await this.context.run(client,async()=>{await client.query('BEGIN');try{await client.query("SELECT set_config('minerva.selection_writer','group-weave-v1',true)");const value=await fn();await client.query('COMMIT');return value;}catch(error){await client.query('ROLLBACK');throw error;}});}finally{client.release();}
  }
  ...
  async save(run:Run){return this.transaction(async()=>{if(run.groupWeave){const ready=await this.query("SELECT 1 FROM pg_trigger WHERE tgrelid='expedition.runs'::regclass AND tgname='selection_writer' AND tgenabled='O' AND pg_get_functiondef(tgfoid) LIKE '%group-weave-v1%'");if(!ready.rowCount)throw new Error('Group Weave worker guard must be installed before submitting');}if(run.selection){const ready=await this.query("SELECT 1 FROM pg_trigger WHERE tgrelid='expedition.runs'::regclass AND tgname='selection_writer' AND tgenabled='O'");if(!ready.rowCount)throw new Error('Selection worker guard must be installed before applying a lens');}await this.query('UPDATE expedition.runs SET value=$2 WHERE id=$1',[run.id,run]);});}
  ...
  async runnable():Promise<Run[]>{return (await this.query("SELECT value FROM expedition.runs WHERE (value->>'status'='running' AND value->>'execution'='worker') OR (value->>'status'='paused' AND value->>'groupWeave' IS NOT NULL) ORDER BY value->>'createdAt' LIMIT 100")).rows.map(r=>r.value);}
  ...
  async candidateForOperation(runId:string,operationId:string):Promise<Candidate|undefined>{return (await this.query("SELECT value FROM expedition.records WHERE run_id=$1 AND kind='candidate' AND value->>'operationId'=$2 ORDER BY seq LIMIT 1",[runId,operationId])).rows[0]?.value;}
  ...
  async control(id:string,action:'pause'|'resume'|'stop'){return this.transaction(async()=>{const r=await this.get(id);if(!r)throw new Error('Run unavailable');if(['stopped','completed'].includes(r.status))return r;if(action==='resume'&&r.groupWeave)throw new Error('Wait for the bounded group Weave to finish before resuming.');r.controlVersion=(r.controlVersion??0)+1;r.status=action==='pause'?'paused':action==='resume'?'running':'stopped';r.reason=action==='stop'?'Stopped by user; completed results retained':undefined;if(action==='stop')for(const a of (await this.attempts(id)).filter(a=>a.status==='reserved')){a.status='cancelled';a.error='Run stopped; external charge may still occur';await this.saveAttempt(a);}if(action==='stop')delete r.groupWeave;await this.save(r);return r;});}
  async reserve(a:Attempt){return this.transaction(async()=>{const r=await this.get(a.runId);if(!r||(a.groupWeaveId?r.status!=='paused'||r.groupWeave!==a.groupWeaveId:r.status!=='running'||!!r.groupWeave)||await this.attempt(a.id))return false;if(a.groupWeaveId&&(a.operation.groupWeave?.requestId!==a.groupWeaveId||(await this.attempts(r.id)).some(prior=>prior.groupWeaveId===a.groupWeaveId&&prior.stage===a.stage)))return false;if(a.sequence!==r.calls+1||(a.maxCallsAtPlanning!==undefined&&a.maxCallsAtPlanning!==r.maxCalls))return false;if((a.selectionAtPlanning??null)!==(r.selection?.id??null)||(a.controlVersionAtPlanning??0)!==(r.controlVersion??0)||(a.corpusVersionAtPlanning??0)!==(r.corpusVersion??0))return false;if(a.stage==='generation'&&r.selection&&a.operation.selection?.configurationId!==r.selection.id)return false;if(a.stage==='generation'&&a.operation.selection)r.active=[...a.operation.selection.activeCandidateIds];if((await this.attempts(r.id)).some(a=>a.status==='reserved'))return false;if(r.calls>=r.maxCalls||r.reservedMicros+a.reservedMicros>r.maxCostMicros){r.status='completed';r.reason='Exploration limit reached';await this.save(r);return false;}r.calls++;r.reservedMicros+=a.reservedMicros;await this.save(r);await this.saveAttempt(a);await this.put('operation',a.operation.id,r.id,a.operation);return true;});}

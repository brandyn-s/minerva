...
export default function LensWorkspace(props: Props) {
  ...
  return <div className="lens-workspace">
    ...
    {(creating || !props.lenses.length) && <form className="lens-create" onSubmit={async e => { e.preventDefault(); setCreationError(""); try { await props.create(name, seeded); setName(""); setCreating(false); } catch (error) { setCreationError(error instanceof Error ? error.message : String(error)); } }}>
  ...
  return <section className="lens-editor" aria-label="Lens editor">
    ...
    {[...r.groups.map(g => ({ ...g, unassigned: false })), { id: "unassigned", label: "Unassigned", members: r.unassigned, representative: null, unassigned: true }].map(g => <section className="lens-group" key={g.id} aria-label={`Group: ${g.label}`}>
      <details open={g.unassigned || !!query}>
        ...
        <MemberList keys={g.members} byKey={byKey} query={query} selected={chosen} toggle={toggle} inspect={async m => { try { const body = await inspect(m); setDetail({ member: m, body }); } catch (e) { setError(e instanceof Error ? e.message : String(e)); } }} />
      </details>
    </section>)}

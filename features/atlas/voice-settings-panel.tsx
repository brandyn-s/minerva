...
import { Button, Summary, Input, Textarea, Select } from "../../components/ui/controls";

import { useMemo, useRef, useSyncExternalStore } from "react";
import FieldGuideHeading from "./field-guide-heading";
import { Settings2, Volume2 } from "../../components/ui/icons";
...
export default function VoiceSettingsPanel({ settings, update, busy, preview, stop, error }: {
  ...
  const select = (key: keyof VoiceSettings, label: string, options: readonly string[]) => <label>{label}<Select value={String(settings[key])} onChange={e => set(key, e.target.value as never)}>{options.map(v => <option key={v} value={v}>{v === "server-vad" ? "Silence detection" : v === "semantic-vad" ? "Completed-thought detection" : key === "voice" ? v[0].toUpperCase() + v.slice(1) : v}</option>)}</Select></label>;
  ...
  return <>
    <Button type="button" className="composer-icon" aria-label="Voice settings" onClick={() => dialog.current?.showModal()}><Settings2 aria-hidden="true" /></Button>
    <dialog ref={dialog} className="voice-settings-dialog field-guide" aria-label="Minerva’s voice">
      ...
      <p className="voice-settings-intro">A voice that fits your thinking.</p>
      <p className="voice-settings-subtitle">Choose how Minerva sounds and responds.</p>
      ...
      <fieldset disabled={busy} className="voice-essentials"><legend>Voice and conversation</legend>
      ...
      <details className="voice-listening voice-conversation"><Summary><span><span>Conversation preferences</span><small>Thinking style, language &amp; personal instructions</small></span></Summary>
      <fieldset disabled={busy}><legend>Conversation preferences</legend>
        <div className="voice-settings-grid">
        ...
        <p className="voice-settings-note">These preferences guide Minerva’s delivery; it can vary.</p>
        <label>Custom instructions<Textarea rows={2} maxLength={3000} value={settings.custom} placeholder="Try: Challenge my assumptions and use concrete examples." onChange={e => set("custom", e.target.value)} /></label>

      </fieldset></details>
      <details className="voice-listening"><Summary><span><span>Listening &amp; microphone</span><small>Pauses, speech recognition &amp; background noise</small></span></Summary><fieldset disabled={busy}><legend>Listening</legend>
      ...
      <Button className="voice-settings-reset" type="button" disabled={busy} onClick={() => update({ ...defaultVoiceSettings })}>Reset defaults</Button>
      </div>
      <footer><div className="voice-settings-footer-actions"><Button type="button" disabled={busy} onClick={preview}><Volume2 aria-hidden="true" />Preview voice</Button><Button type="button" className="voice-settings-done" onClick={() => dialog.current?.close()}>Done</Button></div>
        <p className="voice-settings-note">Preview uses no microphone. Changes apply to your next voice session.</p>
      </footer>
    </dialog>
  </>;
}

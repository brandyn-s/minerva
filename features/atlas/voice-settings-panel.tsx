...
export default function VoiceSettingsPanel({ settings, update, busy, preview, stop, error }: {
  ...
  return <>
    ...
    <dialog ref={dialog} className="voice-settings-dialog field-guide" aria-label="Minerva’s voice">
      ...
      <footer><div className="voice-settings-footer-actions"><button type="button" disabled={busy} onClick={preview}><Volume2 size={16} aria-hidden="true" />Preview voice</button><button type="button" className="voice-settings-done" onClick={() => dialog.current?.close()}>Done</button></div>

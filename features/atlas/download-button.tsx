...
function cardMarkdown(card: Thought, allCards: Thought[], edges: Relationship[], level: number) {
  ...
  const history = `\n${heading}# Revision history\n\n${[...card.revisions].reverse().map(r => `${heading}## Revision ${r.number} — ${r.cause}\n\nTime: ${r.prepared ? "Prepared" : r.time}\n\nTitle: ${r.title}\n\nSummary: ${r.summary}\n\n${r.body}\n\nContribution: ${r.contribution ?? "Historical contribution unknown"}${r.note ? `\n\nModel claim: ${r.note}` : ""}`).join("\n\n") }\n`;
  return `${heading} ${title(card.title)}\n\n${heading}# Summary\n\n${card.summary}\n\n${heading}# Body\n\n${card.body}\n\n${heading}# Contribution\n\n${card.contribution}\n\n${heading}# Relationships\n\n${relationships || "None"}\n${inheritance}${provenance}${history}${weaveMarkdown(card, level)}`;
}

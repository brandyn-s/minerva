...
export default function ExpeditionLenses({ runId, runStatus }: { runId: string; runStatus?: string }) {
  ...
  return <section aria-label="Expedition lenses">
    ...
    {loaded && <LensWorkspace actions={lenses.find(l => l.id === activeId) && <SelectionControls key={activeId} runId={runId} runStatus={runStatus} lens={lenses.find(l => l.id === activeId)!} members={members} />} lenses={lenses} members={members} activeId={activeId} choose={setActiveId} busy={busy} seededLabel="Existing assessor mechanism groups"

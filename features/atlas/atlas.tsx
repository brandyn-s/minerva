...
function Studio({ initial, restoreNotice = "", saveEnabled = true, replace }: { initial?: AtlasSave; restoreNotice?: string; saveEnabled?: boolean; replace?: (save: AtlasSave, notice?: string) => void }) {
  ...
  const [expeditions] = useState<AtlasSave["expeditions"]>(initial?.expeditions ?? []);
  const [activeExpedition] = useState<number | null>(initial?.activeExpedition ?? null);
  ...
  async function importAtlas(merge: boolean) {
    ...
    try {
      ...
      const result = merge ? await mergeAtlas(current, importFile) : { save: importFile, added: importFile.thoughts.length, updated: 0, skipped: 0 };
      ...
      replace?.(result.save, merge ? `Merge complete: ${result.added} added, ${result.updated} updated, ${result.skipped} unchanged. Divergent histories are retained separately.` : "Atlas replaced.");
  ...
  async function generate(feature: LiveFeature, sources: Thought[], contextualMove?: ContextualMove) {
    ...
    try {
      const response = await fetch(`/api/${feature}`, {
        ...
        body: JSON.stringify(feature === "wander" ? { id: sources[0].id, revision: sources[0].revision, title: sources[0].title, summary: sources[0].summary, body: sources[0].body, intent: contextualMove ? "move" : "wander", move: contextualMove } : sources),
      });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || response.statusText);
      ...
      const added = cards.map<CardNode>((card, index) => {
        ...
        return {
          ...
          data: { thought: { ...card, id, revision: 1, revisions: [{ ...firstRevision({ ...card, contribution: feature === "wander" ? `Derived from ${sources[0].title}.` : contributions.join(" ") }, `generated by ${feature === "wander" ? "Wander" : "Weave"}`), ...(result.receipt ? { receipt: receiptSchema.parse(result.receipt) } : {}) }],
  ...
  function inspectExperiment(candidate: Candidate, operation: Operation) {
    const current = latestSave.current.thoughts;
    const result = materialize(candidate, operation, current);
    const material = [...result.sources, result.card];
    const next = current.map(c => material.find(m => m.id === c.id) ?? c);
    for (const c of material) if (!next.some(n => n.id === c.id)) next.push(c);
    latestSave.current = { ...latestSave.current, thoughts: next };
    setNodes(nodes => {
      const updated = nodes.map(n => ({ ...n, data: { ...n.data, thought: next.find(c => c.id === n.id)! } }));
      let x = Math.max(0, ...nodes.map(n => n.position.x)) + 480;
      for (const card of next) if (!nodes.some(n => n.id === card.id)) {
        updated.push({ id: card.id, type: "thought", position: { x, y: 0 }, dragHandle: ".card-grip", ariaLabel: card.title, data: { thought: card } }); x += 480;
      }
      return updated;
    });
    setLiveEdges(edges => [...edges, ...result.edges.filter(e => !edges.some(old => old.id === e.id))]);
    // Materialization does not move the camera or selection; inspect in the existing pane.
    setActive(result.card.id); setPanel("inspect");
  }
  function fit() {
    if (perspective === "Constellation") {
  ...
  return (
    <main className={`studio ${"scalable-atlas"} ${perspective === "Constellation" ? "constellation-atlas" : ""}`}>
      ...
      >
        ...
        >
          <Button iconOnly className="expedition-control thoughts-control" aria-label={`Thoughts ${nodes.length}`} title="Browse thoughts" aria-haspopup="dialog" aria-expanded={panel === "index"} onClick={() => togglePanel("index")}>
          ...
          <Button iconOnly className="expedition-control" aria-label="Read as text" title="Read as text" aria-haspopup="dialog" aria-expanded={panel === "text"} onClick={() => togglePanel("text")}>
            <Image className="scroll-medallion" src="/images/read-scroll.png" width={44} height={44} alt="" />
          </Button>
          {<Button iconOnly className="expedition-control" aria-label="Expedition panel" title="Open expedition panel" aria-haspopup="dialog" aria-expanded={panel === "expedition"} onClick={() => togglePanel("expedition")}><Image src="/images/expedition-compass.png" width={44} height={44} alt="" /></Button>}

          {<details className="layout-menu layout-icon-menu" onKeyDown={event => { if (event.key === "Escape") { event.stopPropagation(); event.currentTarget.open = false; event.currentTarget.querySelector<HTMLElement>("summary")?.focus(); } }}><Summary aria-label="Layout" title="Arrange layout"><Image src="/images/layout-medallion.png" width={48} height={48} alt="" /></Summary>
        ...
        {focusedId && <section className="focus-navigation" aria-label="Focused card connections">
          <div className="focus-heading"><strong>{byId.get(focusedId)?.title}</strong><Button aria-label="Close focused connections" onClick={() => setFocusedId(null)}><X aria-hidden="true" /></Button></div>

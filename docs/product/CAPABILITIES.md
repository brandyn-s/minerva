...
| C07 | Three perspectives and editable lenses | works locally | Lineage/Evolution/Constellation share ideas with independent arrangements and cameras. Explicit Find themes/Regroup propose themes. [Release 2A](../lenses-release-2a.md) adds named atlas/run lenses, exact revision membership, split/merge/move, representatives, undo, full-corpus coverage and save/import preservation. Desktop/touch journeys and a real local API test cover 65-member groups and concurrent stale edits without generation. Lens-driven population selection is not implemented; hosted Neon verification remains open. |

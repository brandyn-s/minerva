...
| C06 | Comparison and Weave | partial | Root demo: two or more selected cards generate one live recombination draft with every parent edge and one-line contributions. A real three-parent Sonnet 5 call and browser error/retry, three-parent lineage and reload-reset checks passed. In memory only; no saved recombination. |
| C07 | Three perspectives | not started | No application implementation |
| C08 | Contextual creative moves | partial | Prepared contextual previews remain. Root selection now offers live Wander for one source and Weave for two or more; calls use AI Gateway OIDC and feature tags. Local real calls and card error/retry checks passed. No contextual planner or other live moves. |

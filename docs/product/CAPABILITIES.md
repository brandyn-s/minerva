...
| C13 | Typed and spoken collaboration | works locally | Root Talk preserves typed Sonnet 5 (`feature:talk`) and adds push-to-talk via AI SDK/Gateway `openai/gpt-realtime-2` (`feature:voice`), confirmed by `gateway.getAvailableModels()`. Voice-only server key mints short-lived client tokens. Permission is requested on press; release sends buffered audio; pressing again stops playback. Heard turns and streamed/spoken replies share the transcript and selected-card context. Mocked denial, setup Retry, delayed setup, interruption, pointer/keyboard hold, dismissal and typed follow-up passed. A real Gateway exchange with a synthetic Chromium microphone is transcribed in HANDOFF; physical microphone/speaker acceptance remains owner-run. |
| C14 | Outputs and reusable results | works locally | Root inspection and Thoughts index Download buttons create client-side Markdown for one card or the entire in-memory atlas, including generated cards and ignoring the index filter. Exports include title, summary, body, decision, evidence and relationships with contributions. Actual Blob contents, download filenames and failure/Retry are verified in the existing browser replay. No route, model call or persistence. |

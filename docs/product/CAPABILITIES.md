...
| C04 | History, inheritance and genome | works locally | Append-only card revisions with time/cause/full text, newest-first word diffs and Revert; frozen sourceRevision with moved-parent indication. Browser save v3, JSON/Markdown history, v1/v2 migration and history-preserving Merge; replay covers edit/revert and round trips. |
| C05 | Branch development and reusable intent | works locally | Develop revises the same card in 1–3 Sonnet 5 calls with reusable saved intents, branch causes, claims and revision badges; Stop retains completed steps. Mock replay covers reuse and cancellation; real two-step Develop and notes recorded in HANDOFF. |
...
| C09 | Creative instruments | works locally | Root Wander, Weave and Expedition call AI Gateway through application routes and retain results in per-browser storage. There is no server persistence or workflow. The existing mocked atlas replay exercises generation, retry and retained results. |
| C10 | Wander | works locally | Root demo: Wander opens suggested next steps for one selected card; Explore freely generates two or three adjacent cards with derivation edges. Local real Sonnet 5 call returned three cards; browser checks cover generation, error/retry and reload reset. In memory only; no durable run or history. |

...
| C07 | Three perspectives and editable lenses | works locally | [Release 2A](../lenses-release-2a.md) provides exact atlas/run membership, editing, undo and independent arrangements. [Release 2B](../lenses-release-2b.md) adds explicit paused population application, protections, deterministic representatives, versioned receipts and applied-lens readings. Native tests, desktop/touch journeys, a real 65-candidate browser/API/SQLite check and isolated Neon concurrency/rollback/older-writer checks pass. No live selection comparison or hosted 2B UI claim. |

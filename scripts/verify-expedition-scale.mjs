...
for(const size of [100,1000,10000]){
 ...
 try{
  ...
  store.transaction(()=>{for(let i=0;i<size;i++){
  ...
  let start=performance.now();const page=store.page(run.id,'candidate',0,30);const pageMs=performance.now()-start;assert.equal(page.items.length,30);assert.equal(page.more,true);
  start=performance.now();const reading=analyze(store,run.id),preview=readingPreview(reading);const analysisMs=performance.now()-start;
  const bytes=Buffer.byteLength(JSON.stringify(preview));assert.ok(bytes<20000);assert.equal(reading.coverage.candidates,size);assert.equal(preview.groups.length,17);assert.ok(preview.groups.every(g=>g.candidates.length<=5));
  results.push({candidates:size,pageSize:page.items.length,pageMs,analysisMs,readingPreviewBytes:bytes});
 }finally{store.close();rmSync(directory,{recursive:true,force:true});}
}
mkdirSync('evaluation-artifacts/transition',{recursive:true});writeFileSync('evaluation-artifacts/transition/scale.json',JSON.stringify({kind:'synthetic local measurements; not model throughput',results},null,2));console.log(JSON.stringify(results));

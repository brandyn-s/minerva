...
try {
    for (const [split, seeds] of Object.entries(protocol.seeds))
        for (const seed of seeds)
            for (const policy of protocol.policies) {
                ...
                let steps = 0;
                while (await tick(store, run.id, fixtureProvider(seed + steps))) {
                    steps++;
                    if (steps > protocol.maxCalls + 1)
                        throw new Error('Unbounded execution');
                }
                const candidates = (await corpus(store, run.id)), assessed = candidates.filter(c => c.assessment), mechanisms = new Set(assessed.map(mechanismKey));
    ...
    const data = { protocol, protocolSha256: createHash('sha256').update(JSON.stringify(protocol)).digest('hex'), results };
    writeFileSync('evaluation-artifacts/transition/comparison.json', JSON.stringify(data, null, 2));
    console.log(JSON.stringify({ runs: results.length, calls: results.reduce((n, r) => n + r.calls, 0), allWithinBudget: results.every(r => r.calls === protocol.maxCalls), output: 'evaluation-artifacts/transition/comparison.json', claim: 'software integration only' }));
}
finally {
    store.close();
